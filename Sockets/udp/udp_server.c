...
int main(int argc, char **argv)
{
  ...
  while (1)
  {
     ...
     */
    bzero(buf, BUFSIZE);
    // check: what is clientaddr before and after recvfrom??
    printf("Client address before recvfrom: %s\n", (char *)&clientaddr.sin_port);
    n = recvfrom(sockfd, buf, BUFSIZE, 0,
                 (struct sockaddr *)&clientaddr, &clientlen); // stores client message in buf, this function blocks
    printf("Client address after recvfrom: %s\n", (char *)&clientaddr.sin_port);

    if (n < 0)
      error("ERROR in recvfrom");

    /*
     * gethostbyaddr: determine who sent the datagram , client
     */
    hostp = gethostbyaddr((const char *)&clientaddr.sin_addr.s_addr, // where is clientAddr initialized???
